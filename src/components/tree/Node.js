import React, {
  Fragment, FC, useState, useRef, useLayoutEffect, cloneElement,
} from 'react';
  ...
  return (
    <>
      ...
      <text
        ...
        ref={refCallback}
        fill={node.depth === 0 ? '#71248e' : node.children && node.children.length > 0 ? (node.data.selected ? '#71248e' : 'white') : ('#71248e')}
      >
        {node.data.name}
      </text>
    </>
